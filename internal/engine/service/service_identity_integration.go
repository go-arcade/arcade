...
func (iis *IdentityIntegrationService) Callback(providerName, state, code string) (*model.Register, error) {
...
func (iis *IdentityIntegrationService) GetProviderByType(providerType string) ([]model.IdentityIntegration, error) {
...
func (iis *IdentityIntegrationService) GetProvider(name string) (*model.IdentityIntegration, error) {
...
func (iis *IdentityIntegrationService) GetProviderList() ([]model.IdentityIntegration, error) {
...
// LDAPLogin LDAP 登录
func (iis *IdentityIntegrationService) LDAPLogin(providerName, username, password string) (*model.Register, error) {
	integration, err := iis.identityRepo.GetProvider(providerName)
	if err != nil {
...
func (iis *IdentityIntegrationService) registerOrLoginUser(providerName string, userInfo *sso.UserInfoAdapter) (*model.Register, error) {
...
func (iis *IdentityIntegrationService) CreateProvider(provider *model.IdentityIntegration) error {
...
func (iis *IdentityIntegrationService) DeleteProvider(name string) error {
	...
	if err := iis.identityRepo.DeleteProvider(name); err != nil {
		log.Errorf("failed to delete provider: %v", err)
		return err
	}
...
func (iis *IdentityIntegrationService) ToggleProvider(name string) error {
	...
	if err := iis.identityRepo.ToggleProvider(name); err != nil {
		log.Errorf("failed to toggle provider: %v", err)
		return err
	}
