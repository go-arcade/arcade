...
import (
	...
	teamrepo "github.com/go-arcade/arcade/internal/engine/repo/team"
...
// CreateTeam 创建团队
func (s *TeamService) CreateTeam(req *team.CreateTeamReq, createdBy string) (*team.TeamResp, error) {
	// 1. 验证组织是否存在
	if req.OrgId == "" {
	...
	// 7. 返回响应
	return team.ToTeamResp(teamEntity), nil
}

// UpdateTeam 更新团队
func (s *TeamService) UpdateTeam(teamId string, req *team.UpdateTeamReq) (*team.TeamResp, error) {
	// 1. 检查团队是否存在
	...
	return team.ToTeamResp(updatedTeam), nil
}
...
func (s *TeamService) GetTeamById(teamId string) (*team.TeamResp, error) {
	teamEntity, err := s.teamRepo.GetTeamById(teamId)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, errors.New("team not found")
...
// ListTeams 查询团队列表
func (s *TeamService) ListTeams(query *team.TeamQueryReq) (*team.TeamListResp, error) {
	// 设置默认分页
	if query.Page <= 0 {
		query.Page = 1
	}
	...
	// 转换为响应格式
	teamResps := make([]*team.TeamResp, 0, len(teams))
	for _, teamEntity := range teams {
		teamResps = append(teamResps, team.ToTeamResp(teamEntity))
	}
...
func (s *TeamService) GetTeamsByOrgId(orgId string) ([]*team.TeamResp, error) {
...
// GetSubTeams 获取子团队
func (s *TeamService) GetSubTeams(parentTeamId string) ([]*team.TeamResp, error) {
...
func (s *TeamService) GetTeamsByUserId(userId string) ([]*team.TeamResp, error) {
