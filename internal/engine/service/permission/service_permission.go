...
import (
	...
	usermodel "github.com/go-arcade/arcade/internal/engine/model/user"
	permissionrepo "github.com/go-arcade/arcade/internal/engine/repo/permission"
	rolerepo "github.com/go-arcade/arcade/internal/engine/repo/role"
...
func (s *PermissionService) GetUserPermissionsWithRoutes(userId string) (*permission.UserPermissions, error) {
...
func (s *PermissionService) CheckPermission(req *permission.PermissionCheckRequest) (*permission.PermissionCheckResponse, error) {
	hasPermission, err := s.PermRepo.HasPermission(req.UserId, req.PermissionCode, req.ResourceId, req.Scope)
	if err != nil {
		return nil, err
	}

	resp := &permission.PermissionCheckResponse{
...
func (s *PermissionService) CreateRoleBinding(req *permission.UserRoleBindingCreate) (*permission.UserRoleBinding, error) {
	...
	// 创建绑定
	binding := &permission.UserRoleBinding{
...
func (s *PermissionService) GetUserRoleBindings(userId string) ([]*permission.UserRoleBinding, error) {
	return s.PermRepo.GetUserRoleBindings(userId)
}

// ListRoleBindings 列出角色绑定（支持多条件查询）
func (s *PermissionService) ListRoleBindings(query *permission.RoleBindingQuery) ([]*permission.UserRoleBinding, int64, error) {
...
func (s *PermissionService) RemoveAllBindingsForResource(scope, resourceId string) error {
	// 获取所有相关用户
	bindings, _, err := s.PermRepo.ListRoleBindings(&permission.RoleBindingQuery{
		Scope:      scope,
		ResourceId: resourceId,
	})
	if err != nil {
...
func (s *PermissionService) validateResource(scope string, resourceId *string) error {
	// Platform 级别不需要资源ID
	if scope == permission.ScopePlatform {
...
// GetRole 获取角色信息
func (s *PermissionService) GetRole(roleId string) (*rolemodel.Role, error) {
...
func (s *PermissionService) MapTeamRoleToProjectRole(teamRoleId string, accessLevel string) string {
	// 根据团队角色和访问级别映射到项目角色
	switch teamRoleId {
	case rolemodel.BuiltinTeamOwner:
		return rolemodel.BuiltinProjectOwner
	case rolemodel.BuiltinTeamMaintainer:
		if accessLevel == "write" {
			return rolemodel.BuiltinProjectMaintainer
		}
		return rolemodel.BuiltinProjectDeveloper
	case rolemodel.BuiltinTeamDeveloper:
		return rolemodel.BuiltinProjectDeveloper
	case rolemodel.BuiltinTeamReporter:
		return rolemodel.BuiltinProjectReporter
	case rolemodel.BuiltinTeamGuest:
		return rolemodel.BuiltinProjectGuest
	default:
		return rolemodel.BuiltinProjectGuest
	}
}
