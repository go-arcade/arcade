...
func (rs *RoleService) CreateRole(req *model.CreateRoleRequest) (*model.Role, error) {
	// validate scope
	if req.Scope != model.RoleScopeProject && req.Scope != model.RoleScopeTeam && req.Scope != model.RoleScopeOrg {
	...
	role := &model.Role{
		...
		Scope:       req.Scope,
		OrgId:       req.OrgId,
		IsBuiltin:   model.RoleCustom, // custom role
...
func (rs *RoleService) UpdateRole(roleId string, req *model.UpdateRoleRequest) error {
	// get existing role
	...
	if role.IsBuiltin == model.RoleBuiltin {
...
func (rs *RoleService) ToggleRole(roleId string) error {
	...
	newStatus := model.RoleDisabled
	if role.IsEnabled == model.RoleDisabled {
		newStatus = model.RoleEnabled
	}

	if err := rs.roleRepo.EnableRole(roleId, newStatus == model.RoleEnabled); err != nil {
...
func (rs *RoleService) ListRoles(req *model.ListRolesRequest) (*model.ListRolesResponse, error) {
	...
	return &model.ListRolesResponse{
		Roles:    roles,
		Total:    total,
		PageNum:  req.PageNum,
