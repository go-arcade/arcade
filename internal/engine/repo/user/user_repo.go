...
type IUserRepository interface {
	AddUser(addUserReq *user.AddUserReq) error
	UpdateUser(userId string, u *user.User) error
	FetchUserInfo(userId string) (*user.UserInfo, error)
	GetUserByUsername(username string) (string, error)
	Login(login *user.Login) (*user.User, error)
	Register(register *user.Register) error
	...
	SetLoginRespInfo(accessExpire time.Duration, loginResp *user.LoginResp) error
...
func (ur *UserRepo) FetchUserInfo(userId string) (*user.UserInfo, error) {
	ctx := context.Background()
	key := consts.UserInfoKey + userId
	u := &user.UserInfo{UserId: userId}
...
func (ur *UserRepo) Login(login *user.Login) (*user.User, error) {
	var u = &user.User{}
...
func (ur *UserRepo) SetLoginRespInfo(accessExpire time.Duration, loginResp *user.LoginResp) error {
	if ur.cache == nil {
		return fmt.Errorf("cache not available")
	}
	ctx := context.Background()
