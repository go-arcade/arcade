...
type IdentityIntegrationRepo struct {
	db                                      database.DB
	IdentityIntegrationidentity_integration identity_integration.IdentityIntegration
}
...
func (ii *IdentityIntegrationRepo) GetProvider(name string) (*identity_integration.IdentityIntegration, error) {
	...
	if err := ii.db.DB().Table(ii.IdentityIntegrationidentity_integration.TableName()).
		Where("name = ?", name).
...
func (ii *IdentityIntegrationRepo) CreateProvider(provider *identity_integration.IdentityIntegration) error {
	return ii.db.DB().Table(ii.IdentityIntegrationidentity_integration.TableName()).Create(provider).Error
}
...
func (ii *IdentityIntegrationRepo) UpdateProvider(name string, provider *identity_integration.IdentityIntegration) error {
	return ii.db.DB().Table(ii.IdentityIntegrationidentity_integration.TableName()).
		Where("name = ?", name).
...
func (ii *IdentityIntegrationRepo) ProviderExists(name string) (bool, error) {
	var count int64
	err := ii.db.DB().Table(ii.IdentityIntegrationidentity_integration.TableName()).
		Where("name = ?", name).
		Count(&count).Error
	return count > 0, err
}
